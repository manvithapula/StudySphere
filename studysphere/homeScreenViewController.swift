    ...
    private func updateStreakDisplay() {
          ...
          for (index, streakImageView) in streaks.enumerated() {
              if index < currentStreak {
                  streakImageView.image = UIImage(systemName: "flame")  // Streak day marked with flame
              } else {
                  streakImageView.image = UIImage(systemName: "circle.dotted")  // Non-streak day
              }
          }

          // Update the streak label to show the number of consecutive streak days
